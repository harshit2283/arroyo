...
use arroyo_types::{
    from_micros, range_for_server, server_for_hash, to_micros, CheckpointBarrier, Data, Key,
    Message, Record, TaskInfo, Watermark, WorkerId, BYTES_RECV, BYTES_SENT, MESSAGES_RECV,
    MESSAGES_SENT,
};
...
impl RunningEngine {
    ...
    pub fn operator_controls(&self, operator_id: &str) -> Vec<Sender<ControlMessage>> {
        self.program
            .graph
            .node_indices()
            .filter(|idx| {
